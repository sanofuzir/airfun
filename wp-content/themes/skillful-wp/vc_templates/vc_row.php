...
if ( $type != 'in_container' ) {
	$output .= '<div class="'.$type_class.' '.$parallax.''.$video.''.$el_class.vc_shortcode_custom_css_class($css, ' ').'" '.$style.''.$data.'>';
	$style = '';
}

$output .= $bg_video;
$output .= '<div class="'.$css_class.' '. (( $type !== 'in_container' ) ? '' : $parallax.$el_class.vc_shortcode_custom_css_class($css, ' ')) .'" '. (( $type != 'in_container' ) ? '' : $data) .' '.$style.'>';
