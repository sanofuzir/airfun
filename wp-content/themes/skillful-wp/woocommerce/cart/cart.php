...
<div class="row">
	<form action="<?php echo esc_url( WC()->cart->get_cart_url() ); ?>" method="post" class="small-12 columns">
						...
						foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
							...
							<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
								...
								<td class="product-remove">
									<?php
										echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf( '<a href="%s" class="remove" title="%s"><i class="icon-budicon-543"></i></a>', esc_url( WC()->cart->get_remove_url( $cart_item_key ) ), __( 'Remove this item',THB_THEME_NAME ) ), $cart_item_key );
									?>
								</td>
							</tr>
							<?php
		...
		<div class="row">
			<div class="small-12 medium-7 columns">
				<div class="left-side">
					<?php if ( WC()->cart->coupons_enabled() ) { ?>
						<div class="row">
							<div class="small-12 medium-7 columns">
								<input type="text" name="coupon_code" id="coupon_code" value="" class="full" placeholder="<?php _e( 'Enter Coupon', THB_THEME_NAME ); ?>"/>
							</div>
							<div class="small-12 medium-5 columns">
								<input type="submit" class="btn outline full" name="apply_coupon" value="<?php _e( 'Apply Coupon',THB_THEME_NAME ); ?>" />
							</div>
						</div>
			...
			<div class="small-12 medium-5 columns">
				<div class="right-side">
					<input type="submit" class="button outline white full" name="update_cart" value="<?php _e( 'Update',THB_THEME_NAME ); ?>" />
					<input type="submit" class="checkout-button button black wc-forward" name="proceed" value="<?php _e( 'Proceed to Checkout',THB_THEME_NAME ); ?>" />
